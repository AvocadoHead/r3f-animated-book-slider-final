...
export const buildDefaultBookPages = async (urls) => {
  ...
  // Last leaf: front has last content, back is back cover (no blank page)
  const lastContentIndex = contentLayouts.length - 1;
  if (lastContentIndex > 0 && layoutIndex <= lastContentIndex) {
    leaves.push({
      front: contentLayouts[lastContentIndex] || blankLayout,
      back: { texture: backTexture, fabricJSON: null }
    });
  } else {
    // If no remaining content, just use back cover
    leaves.push({
      front: blankLayout,
      back: { texture: backTexture, fabricJSON: null }
    });
