...
// CORRECT IMPORT for src/components/editor/EditorCanvas.jsx
...
export const EditorCanvas = ({ initialData, onSave, onClose }) => {
  ...
  useEffect(() => {
    ...
    const canvas = new fabric.Canvas(canvasRef.current, {
      ...
      backgroundColor: 'transparent', // Transparent so CSS background shows
      preserveObjectStacking: true,
    });

    fabricCanvasRef.current = canvas;

    // Load Objects (Layers) if they exist
  ...
  const handleSave = () => {
      ...
      // Merge Background + Objects for final texture
      if (initialData.texture) {
          fabric.Image.fromURL(initialData.texture, (bgImg) => {
              ...
              fabricCanvasRef.current.remove(bgImg); // Remove so it doesn't stay in editable layer
      ...
      <div className="bg-white rounded-xl shadow-2xl overflow-hidden flex flex-col" style={{ width: PAGE_DIMENSIONS.width + 100, height: '95vh' }}>
        ...
        <div className="flex-1 bg-gray-600 overflow-auto flex justify-center p-8">
           <div className="relative shadow-2xl bg-white" 
                style={{ 
                    width: PAGE_DIMENSIONS.width, 
                    height: PAGE_DIMENSIONS.height,
                    // FAIL-SAFE BACKGROUND: Ensure the image always shows
        ...
        <div className="p-4 bg-white border-t flex justify-between items-center">
            <div className="text-xs text-gray-500">
                Background locked. Add text/stickers on top.
            </div>
