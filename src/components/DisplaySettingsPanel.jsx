    ...
    <div className="relative">
      ...
      {isOpen && (
        <div className="absolute top-12 right-0 bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl p-4 min-w-[240px] border border-gray-200 animate-in fade-in slide-in-from-top-2 duration-200 z-50">
          ...
          {/* Reflections (Environment Intensity) */}
          <div className="mb-3">
            <div className="flex justify-between items-center mb-1">
              <label className="text-xs text-gray-500">Reflections</label>
              <span className="text-xs text-gray-400">{displaySettings.envIntensity === 0 ? 'Off' : Math.round(displaySettings.envIntensity * 100) + '%'}</span>
            </div>
            <input
              type="range"
              min="0"
              max="1"
              step="0.1"
              value={displaySettings.envIntensity ?? 0.5}
          ...
          {/* Reset button */}
          <button
            onClick={() => setDisplaySettings({ brightness: 1.0, envIntensity: 0.5, wobble: 0.2 })}
